...
export class Design {
     ...
     */
    public set hostOverride(hostCSS: string | undefined) {
        this.hostOverride_ = hostCSS;
    }
...
${selector ? `.${selector}` : ":host"} {${this.compileCSS(css)}}\n`;
        ...
        if (
            css.media &&
            typeof css.media === "object" &&
            !Array.isArray(css.media)
        ) {
        ...
        if (
            css.keyframes &&
            typeof css.keyframes === "object" &&
            !Array.isArray(css.keyframes)
        ) {
    ...
    private compileCSS(css: CSSConfig): string {
        ...
        for (let key in css) {
            ...
            cssString += `\n  ${this.americanise(propKey)}: ${this.americanise(
                propValue
            )};`;
        }
    ...
    private compileKeyframes(config: CSSConfig): string {
        ...
        const blocks = Object.entries(keyframes).map(([key, prop]) => {
            const props = Object.entries(prop)
                .map(([rawKey, rawVal]) => {
                    const { propKey, propValue } = this.parseProperties(
                        rawKey,
                        rawVal
                    );
    ...
    private compileMedia(
        ...
        const bp = Object.entries(media).find(([k]) =>
            k.toLowerCase().endsWith("bp")
        );
        if (!bp) throw new Error("Media config must include a *Bp key");

        const [breakKey, breakVal] = bp as [string, CSSValue];

        const { propKey: headingProp, propValue: headingVal } =
            this.parseBreakpoint(breakKey, breakVal);
